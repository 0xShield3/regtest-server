...
module.exports = function (router, callback) {
    function addressWare(req, res, next) {
        ...
        try {
            let script = !address.match(/^[0-9a-f]+$/i) ? bitcoin.address.toOutputScript(address, NETWORK) : Buffer.from(address, 'hex')
    ...
    router.get('/a/alt/:address/unspents', addressWare, async (req, res) => {
        ...
        try {
            ...
            const results = unspents.map((unspent) => {
                const script = !!unspent.address ? bitcoin.address.toOutputScript(unspent.address, NETWORK) : null
                return {
    ...
    router.post('/p/rpc', bodyParser.json(), (req, res) => {
        rpc(req.body.method, req.body.params || [], (err, result) => {
            ...
            res.easy(null, { result, id: req.body.id })
        })
    })
